...
db.restaurants.find({grades: {$elemMatch: {score: {$not: {$gt:10}}}}}, {restaurant_id:1, name:1, borough:1, cuisine:1, _id:0}) 
db.restaurants.find({$or: [{name: /^Wil/},{$and: [{cuisine: "Seafood"},{cuisine: {$nin: ["American ", "Chinese"]}}]}]}, {restaurant_id:1, name:1, borough:1, cuisine:1, _id:0}) 
db.restaurants.find({grades: {$elemMatch: {score: {$eq: 11}, grade: {$eq: "A"},date: {$eq: ISODate("2014-08-11T00:00:00Z")}}}}, {restaurant_id:1, name:1, grades:1, _id:0})
db.restaurants.find({"grades.1.score": {$eq: 9}, "grades.1.grade": {$eq: "A"}, "grades.1.date": {$eq: ISODate("2014-08-11T00:00:00Z")}}, {restaurant_id:1, name:1, grades:1, _id:0})
db.restaurants.find({"address.coord.1": {$gt: 42, $lt:52}}, {restaurant_id:1, name:1, "address.street":1, "address.coord":1 , _id:0})
